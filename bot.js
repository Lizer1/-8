...
const embed = new Discord.RichEmbed()
                   ...
                   .setColor("RANDOM")
                .setFooter(` ©   Toxic Shop `)
                .setTitle("  Toxic Shop Left Server !")
...
client.on('guildDelete', guild => {
  ...
  .setTitle("**__Why  Toxic Shop Left ?__**")
  .setColor("ffffff")
  .setDescription("**We Are Sorry For Any Problems In The Bot **")
   .addField("🌍 **| CONTACT :**   ",
" `-contact` `-sug` `-bug` ")
 .addField('🔗 **| SUPPORT SERVER** :' , "  https://discord.gg/https://discord.gg/6kEjkB7")
            .setFooter(` Toxic Shop`, 'https://cdn.discordapp.com/attachments/402070843032731648/417407034775044097/image.jpg')
      guild.owner.send(star)
})

client.on("guildCreate", guild => {
console.log(`  Toxic Shop Added From Server -- = ${guild.name} = -- , Server Owner -- = ${guild.owner.user.username} = --`)
...
const embed = new Discord.RichEmbed()
                   ...
                   .setColor("RANDOM")
                .setFooter(` ©   Toxic Shop `)
                .setTitle("  Toxic Shop Added To Server !")
                .addField("**Guild Name:**", guild.name )
                 .addField("**Owner:**", guild.owner.user.username )
                 .addField("**MEMBER COUNT:**", g )
                  .setThumbnail (i)
...
.setAuthor(message.author.username, message.author.avatarURL)
.setTitle(':arrow_right: Click Here Invite Link اضغط هنا')
.setURL('  https://discord.gg/https://discord.gg/6kEjkB7 ')
  .setFooter(`©   Toxic Shop `)
message.channel.sendEmbed(embed);
...
client.on('message', message => {
    ...
    .addField("**🆔 [  Toxic Shop ID ] **", "**"+"500702351229714432"+"**",true)
    .addField("**🏷 [  Toxic Shop ] **","** دريمس**")
    ...
    .setColor("ffffff")
    .setFooter(`-help to see all  Toxic Shop commands `, ' https://cdn.discordapp.com/attachments/409281937794924546/420678602892378112/Untitled-1_converted.png')
    message.channel.sendEmbed(embed)
    }
    });
...
const games = JSON.parse(fs.readFileSync('./ Toxic Shop/Storage/games/games.json', "utf8"));
client.on("message", message => {
    if (!message.content.startsWith(prefix)) return;
  if (message.author.bot) return;
...
fs.writeFile('./ Toxic Shop/Storage/games/games.json', JSON.stringify(games), (err) => {
...
const q = JSON.parse(fs.readFileSync('./ Toxic Shop/Storage/games/ques.json', 'utf8'));
client.on('message', message => {
  if(!message.channel.guild) return;
...
const a = JSON.parse(fs.readFileSync('./ Toxic Shop/Storage/games/amthal.json', 'utf8'));
client.on('message', message => {
  if(!message.channel.guild) return;
...
const b = JSON.parse(fs.readFileSync('./ Toxic Shop/Storage/games/a3lam.json', 'utf8'));
client.on('message', message => {
  if(!message.channel.guild) return;
...
const e = JSON.parse(fs.readFileSync('./ Toxic Shop/Storage/games/emoji.json', 'utf8'));
client.on('message', message => {
  if(!message.channel.guild) return;
...
var c = `${w[Math.floor(Math.random() * w.length)]}`
    const f = JSON.parse(fs.readFileSync('./ Toxic Shop/Storage/games/fkk.json', 'utf8'));
   if(message.content.startsWith('-فكك')) {
         if(!message.channel.guild) return;

const w = ['./ Toxic Shop/img/fkk.png'];
...
const w = ['./ Toxic Shop/img/sarahah.png'];
var Canvas = require('canvas')
var jimp = require('jimp')
...
var jimp = require('jimp')
  ...
  if (curLevel > userData.level) {
    ...
    let Image = Canvas.Image,
            ...
            ctx = canvas.getContext('2d');       
        fs.readFile('./ Toxic Shop/img/levll.png', function (err, Background) {
            if (err) return console.log(err);
            let BG = Canvas.Image;
            let ground = new Image;
            ground.src = Background;
...
const w = ['./ Toxic Shop/img/profil.png'];
var Canvas = require('canvas')
var jimp = require('jimp')
...
client.on('message', message => {
   if(message.content == ('-هويتي')) {    
    ...
    const w = ['./ Toxic Shop/img/ID1.png','./ Toxic Shop/img/ID2.png','./ Toxic Shop/img/ID3.png','./ Toxic Shop/img/ID4.png','./ Toxic Shop/img/ID5.png'];
...
const w = ['./ Toxic Shop/img/img.png']
...
client.on('message', message => {
   ...
   const q = new Discord.RichEmbed()
     .setImage("https://cdn.discordapp.com/attachments/409331262025891841/421426082475147264/ Toxic Shop-bot-help.png")
...
client.on('message', message => {
    ...
    const embed = new Discord.RichEmbed()
                   .setColor("ffffff")
                .setFooter(` ©   Toxic Shop `)
                    .setAuthor(" Toxic Shop" ,"https://cdn.discordapp.com/attachments/409331262025891841/428511194832437251/Untitled-1_converted.png" )
.setDescription(`
...
message.react("📩")
var embed = new Discord.RichEmbed()
 .setAuthor(" Toxic Shop" ,"https://cdn.discordapp.com/attachments/499213385750282244/502842976095961099/no_u.png" )
...
🔗 **| سيرفر الدعم و المساعدة** :  https://discord.gg/https://discord.gg/6kEjkB7
`)
.setColor('0bbaba')
  .setFooter(`©   Toxic Shop `)
message.author.sendEmbed(embed)
...
if(!message.channel.guild) return;
const w = ['./ Toxic Shop/img/colors.png'];
var Canvas = require('canvas')
var jimp = require('jimp')
     let Image = Canvas.Image,
